...
static void AddConfig(ConfigItem * target, int *id, char *config,
                        float val, char *str, enum configValueType type,
                        char *comment)
{
...
void configurationWrite(FILE * target)
{
    ...
    for (group = 0; group < NUM_CONFIG_GROUPS; ++group) {
        ...
        for (i = 0; i < config_list[group].list_size; ++i) {
            ...
            case TYPE_FLOAT:
                fprintf(target, "%f",
                         config_list[group].list[i].val.real);
                break;
            case TYPE_BOOLEAN:
                fprintf(target, "%s",
                         config_list[group].list[i].val.num !=
                         0 ? "true" : "false");
                break;
            ...
            case TYPE_STRING:
                fprintf(target, "\"%s\"",
                         config_list[group].list[i].val.str);
            default:
                break;
            }
            if (config_list[group].list[i].comment != NULL)
                fprintf(target, " \t# %s",
                         config_list[group].list[i].comment);
            fprintf(target, "\n");
        }
        fprintf(target, "\n");
    }
...
int configSet(char *str, configValue val)
{
    ...
    for (i = 0; i < config_list[config_currentGroup].list_size; ++i) {
        if (strcasecmp(config_list[config_currentGroup].list[i].name, str)
             == 0) {
